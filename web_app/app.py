...
def get_datatime(np_array):
    ...
    return render_template("graphics.html", np_array=np_array, days=days, hours=hours, number_sensor=number_sensor, title="graphics")
    ...
    for i in which_data:
        ...
        for data in arr[1:, i]:
            ...
            if date_from.date() <= current_date.date() <= date_to.date():
                ...
                tmp_labels.append(current_date)
            id = id + 1
...
def plotline():
    ...
    return render_template('lineChart.html', title='line chart', options=option, sensors=sensor, labels=dl[0], values=dv[0], names=names[0])
